...
public class GeneralLogs {
    public void createGeneralLogs(String userId, String logMessage) throws IOException {
        String general_log="";
        File f = new File("resources\\GeneralLogs.txt");
        if(f.exists()) {
            BufferedReader br = new BufferedReader(new FileReader(f));
            if (br.readLine() == null) {
                general_log = "User|Log Message|TimeStamp\n";
            }
        }
        else{
            general_log = "User|Log Message|TimeStamp\n";
        }
        general_log = general_log+userId+"|"+logMessage+"|"+new Date();
